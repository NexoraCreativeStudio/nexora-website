...
/* Provider environments. TEST/SANDBOX evidence must never be accepted as
   Production evidence. */
...
export function applyReconciliation(payment, invoice, reconciliation, opts = {}) {
  ...
  if (rec.outcome === 'EXACT') {
    ...
    if (next.amount_remaining <= 0) {
      ...
      next.audit_events.push({ event: 'reconciled_partial', at, event_id: sha256hex(`nexora-payment:${payment.payment_id}:recpart:${at}`).slice(0, 16), detail: `Partial payment ${rec.amount} ${rec.currency} received; ${next.amount_remaining} remaining.` });
    }
...
/* ------------------------------------------------------------------ */
/* Scanners re-exported for the validator                            */
/* ------------------------------------------------------------------ */
export { sha256hex, scanSecrets, scanBankDetails, scanPaymentLink, scanLegacy, scanVatAssertions, scanFinancialClaims, scanPathLeakage, collectStrings };
